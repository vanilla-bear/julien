...
class YamlFormLocation extends YamlFormCompositeBase {
  ...
  public static function processYamlFormComposite(&$element, FormStateInterface $form_state, &$complete_form) {
    ...
    if (empty($google_api)) {
      ...
      $element['script'] = [
//        '#markup' => '<script src="https://maps.googleapis.com/maps/api/js?key=' . $api_key . '"></script>',
        '#markup' => '<script src="https://maps.googleapis.com/maps/api/js?key=' . $api_key . '&callback=initMap"></script>',
        '#allowed_tags' => ['script'],
      ];
      $google_api = TRUE;
    }
